...
ngx_open_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    ngx_log_stderr(0,"%s,%d------------------",__FUNCTION__,__LINE__);
    log = cycle->log;
    ngx_log_stderr(0,"begin ngx_open_listening_sockets");
    ...
    for (tries = 5; tries; tries--) {
        ...
        ngx_log_stderr(0,"%s,%d,cycle->listening.nelts=%d",__FUNCTION__,__LINE__,cycle->listening.nelts);
        ls = cycle->listening.elts;
        for (i = 0; i < cycle->listening.nelts; i++) {
...
#endif
            /* TODO: close on exit */
            ngx_log_stderr(0,"%s,%d,ngx_event_flags=0x%xi",__FUNCTION__,__LINE__,ngx_event_flags);
...
#endif
            ngx_log_stderr(0,"listen() to %V,backlog %d",&ls[i].addr_text,ls[i].backlog);
