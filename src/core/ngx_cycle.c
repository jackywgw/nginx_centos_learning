...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    ngx_log_error(NGX_LOG_DEBUG,log,0,"conf_prefix=%s",cycle->conf_prefix.data);
    ...
    ngx_log_error(NGX_LOG_DEBUG,log,0,"prefix=%s",cycle->prefix.data);
    ...
    ngx_log_error(NGX_LOG_DEBUG,log,0,"conf_file=%s",cycle->conf_file.data);
    ...
    ngx_log_error(NGX_LOG_DEBUG,log,0,"conf_param=%s",cycle->conf_param.data);
    /*默认old_cycle里面没有paths*/
    n = old_cycle->paths.nelts ? old_cycle->paths.nelts : 10;

    ngx_log_error(NGX_LOG_DEBUG,log,0,"paths.nelts=%d,n=%d",old_cycle->paths.nelts,n);
    ...
    ngx_log_error(NGX_LOG_DEBUG,log,0,"shared_memory.part.nelts=%d,n=%d",old_cycle->shared_memory.part.nelts,n);
    ...
    conf.ctx = cycle->conf_ctx;/*cf->ctx*/
    conf.cycle = cycle;
    conf.pool = pool;
    conf.log = log;
    ...
    ngx_log_error(NGX_LOG_DEBUG,log,0,"ending ngx_conf_parse------------------------");
    ...
    /*由于解析了配置文件，而配置文件里面有listen 8080,所以此次的listening数组中已使用的个数为1*/ 
    ngx_log_error(NGX_LOG_DEBUG,log,0,"%s,%d------------------cycle->listening.nelts=%d",__FUNCTION__,__LINE__,cycle->listening.nelts);
    ngx_log_error(NGX_LOG_DEBUG,log,0,"ngx_process=%d",ngx_process);
    /*ngx_process全局变量默认为0，如果是进程被信号kill掉了，则此处ngx_process为NGX_PROCESS_SIGNALLER*/
    if (ngx_process == NGX_PROCESS_SIGNALLER) {
        return cycle;
    }
    /*通过ngx_core_module模块的索引值index，获取ngx_core_module模块的配置结构*/
    ccf = (ngx_core_conf_t *) ngx_get_conf(cycle->conf_ctx, ngx_core_module);
    ngx_log_stderr(0,"%s,%d,ngx_test_config=%d",__FUNCTION__,__LINE__,ngx_test_config);
    if (ngx_test_config) {
    ...
    ngx_log_error(NGX_LOG_DEBUG,log,0,"%s,%d",__FUNCTION__,__LINE__);
    ...
    if (old_cycle->listening.nelts) {
        ngx_log_error(NGX_LOG_DEBUG,log,0,"++++++++++++++++++++++++old_cycle->listening.nelts=%d",old_cycle->listening.nelts);
        ...
        ngx_log_error(NGX_LOG_DEBUG,log,0,"------------------cycle->listening.nelts=%d",cycle->listening.nelts);
    ...
    ngx_log_error(NGX_LOG_DEBUG,log,0,"%s,%d------ngx_test_config=%d------------",__FUNCTION__,__LINE__,ngx_test_config);
    if (!ngx_test_config) {
